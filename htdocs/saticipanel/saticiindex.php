...
// 2. Toplam ürün
$urunSorgu = $db->prepare("SELECT COUNT(*) as toplam_urun FROM urunler WHERE satici_id = :id AND silindi_mi=0");
...
if ($arama != "") {
    ...
    $sorgu = $db->prepare("SELECT * FROM urunler WHERE satici_id = :satici_id AND silindi_mi=0");
...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="search-and-add-container mb-6">
        <!-- Ürün Arama Kutusu -->
        <!-- Ürün Arama Kutusu -->
        <form method="GET" class="flex w-full sm:w-2/3 gap-2">
            <input type="text" name="arama" placeholder="Ürün adı ara..." 
                class="border rounded px-3 py-2 focus:outline-none w-full" 
                value="<?php echo isset($_GET['arama']) ? htmlspecialchars($_GET['arama']) : ''; ?>">
            
            <button type="submit" class="bg-blue-600 text-white px-4 rounded hover:bg-blue-700">Ara</button>

            <a href="saticiindex.php" class="bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600">Temizle</a>
        </form>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <?php foreach ($urunler as $urun) { ?>
            <div class="bg-white rounded-lg shadow p-4 relative">
                <img src="<?php echo htmlspecialchars($urun['resim_url'] ?? 'https://via.placeholder.com/300x200'); ?>" class="w-full h-48 object-cover mb-2 rounded">
                ...
                <div class="absolute bottom-2 right-2 flex gap-2">
                    <a href="urun_duzenle.php?urun_id=<?php echo $urun['urun_id']; ?>"
                    class="bg-blue-500 hover:bg-blue-600 text-white text-sm py-1 px-3 rounded shadow">
                    Düzenle
                    </a>
                    <a href="urun_sil.php?urun_id=<?php echo $urun['urun_id']; ?>"
                    onclick="return confirm('Silmek istediğinize emin misiniz?');"
                    class="bg-red-500 hover:bg-red-600 text-white text-sm py-1 px-3 rounded shadow">
                    Sil
                    </a>
                </div>
            </div>
