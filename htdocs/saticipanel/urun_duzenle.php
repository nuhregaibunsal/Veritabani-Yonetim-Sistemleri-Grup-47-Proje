...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Ürünü güncelle
    $guncelle = $db->prepare("UPDATE urunler SET ad = :ad, aciklama = :aciklama, fiyat = :fiyat, stok = :stok, resim_url = :resim_url, aktif = :aktif WHERE urun_id = :urun_id AND satici_id = :satici_id");
    $guncelle->execute([
        ':ad' => $ad,
        ':aciklama' => $aciklama,
        ':fiyat' => $fiyat,
...
<div class="form-container bg-white p-6 rounded-lg shadow-md">
    ...
    <form method="POST" action="" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            <label class="block font-semibold">Ürün Durumu</label>
            <div class="flex gap-4 mt-2">
                <label class="inline-flex items-center">
                    <input type="radio" name="aktif" value="1" <?= $urun['aktif'] == 1 ? 'checked' : '' ?>>
                    <span class="ml-2">Aktif</span>
                </label>
                <label class="inline-flex items-center">
                    <input type="radio" name="aktif" value="0" <?= $urun['aktif'] == 0 ? 'checked' : '' ?>>
